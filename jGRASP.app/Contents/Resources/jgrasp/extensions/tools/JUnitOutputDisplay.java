...
public class JUnitOutputDisplay implements Runnable {
   ...
   private class ProgressPanel extends JPanel {
      ...
      public void paintComponent(final Graphics g) {
         ...
         g.setColor(Drawing.getColor(Colors.HL_GREEN));
         int l = fHt / 2;
         int t = fHt / 2 + fHt;
         if (successWidth > 0) {
         ...
         if (ignoredWidth > 0) {
            g.setColor(Drawing.createColor(Color.ORANGE));
         ...
         if (failWidth > 0) {
            g.setColor(Drawing.getColor(Colors.HL_RED));
         ...
         g.setColor(Drawing.getColor(Colors.FG));
   ...
   private void processLine(final String line) {
      ...
      SwingUtilities.invokeLater(()->processLineEDT(line));
   }
      ...
      if (frame == null) {
         GIconData iconData = 
            new GIconData() {
               ...
               public Paint getBackgroundPaint(final int x, final int y,
                     final int size) {
                  float[] fractions = new float[] { 0, .5f, 1f };
                  Color[] colors = new Color[] { Drawing.getColor(Colors.HL_RED),
                        Drawing.getColor(Colors.WHITE), Drawing.getColor(Colors.HL_GREEN) };
                  return new LinearGradientPaint(
                        x + size / 5, y + size / 5, x + size - size / 5,
                        y + size - size / 5, fractions, colors);
               }
         ...
         StyleConstants.setForeground(textStyle, Drawing.getColor(Colors.FG));
      ...
      for (int l = 0; l < lines.length; l++) {
         ...
         try {
            ...
            if (line.startsWith(headerMarker)) {
               ...
               StyleConstants.setForeground(boldStyle, Drawing.getColor(Colors.FG));
