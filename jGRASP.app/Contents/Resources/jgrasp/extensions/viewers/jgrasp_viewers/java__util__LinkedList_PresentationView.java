...
public class java__util__LinkedList_PresentationView
   ...
   /** True if the object being viewed is a Java 1.7 LinkedList, false if it is
    * from an earlier version of Java. **/
   private boolean v17;
   ...
   public java__util__LinkedList_PresentationView(final ViewerCreateData vcd) {
      super(vcd, LINKED | BACK_LINKS | HEADER | END_LINK_TO_START
            | START_LINK_TO_END);
      try {
   ...
   /** Gets the list node ("entry" in java.util.LinkedList terminology) at a
    * specified index.
    *
   ...
   private Value getEntry(final Value value, final int index,
      ...
      if (v17) {
         getMethod = value.getMethod(context, "node",
               "java.util.LinkedList$Node", new String[]
               { "int" });
      }
      else {
         getMethod = value.getMethod(context, "entry",
               "java.util.LinkedList$Entry", new String[]
               { "int" });
      }
