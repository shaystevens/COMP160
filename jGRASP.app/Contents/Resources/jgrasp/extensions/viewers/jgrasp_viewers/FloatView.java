...
public abstract class FloatView extends ViewerRoot {
   ...
   /** Labels for showing the sign, exponent, and mantissa in binary, hex, and
    * decimal, and the header labels for those categories. **/
   ...
   /** Index of the sign bit for the float type that this viewer displays. **/
   private int signBit;

   /** Index of the first exponent bit for the float type that this viewer
    * displays. **/
   private int firstExponentBit;

   /** Index of the last exponent bit for the float type that this viewer
    * displays. **/
   private int lastExponentBit;

   /** Index of the first mantissa bit for the float type that this viewer
    * displays. **/
   private int firstMantissaBit;

   /** Index of the last mantissa bit for the float type that this viewer
    * displays. **/
   private int lastMantissaBit;

   /** Bias to be subtracted from the exponent bits value to get the actual
    * exponent. **/
    ...
    * viewer displays.
    *
    * @param firstExponentBitIndex the index of the first exponent bit for the
    * float type that this viewer displays.
    *
    * @param lastExponentBitIndex the index of the last exponent bit for the
    * float type that this viewer displays.
    *
    * @param firstMantissaBitIndex the index of the first mantissa bit for the
    * float type that this viewer displays.
    *
    * @param lastMantissaBitIndex the index of the last mantissa bit for the
    * float type that this viewer displays.
    *
    * @param exponentBiasValue the bias to be subtracted from the exponent bits
    * value to get the actual exponent. **/
   public FloatView(final int signBitIndex, final int firstExponentBitIndex,
    ...
    *
    * @return a text representation for the value that is similar in form to
   ...
   private static String getBDText(final BigDecimal val, final String ref) {
      ...
      if (dotPos >= 0) {
         ...
         int zeroCut = end;
         while (zeroCut > dotPos + 2 && result.charAt(zeroCut - 1) == '0') {
            zeroCut--;
         }
         if (zeroCut < end) {
   ...
   public void buildGui(final JPanel mainPanel) {
      ...
      for (int row = 0; row < 4; row++) {
         ...
         for (int col = 0; col < 3; col++) {
            ...
            if (row >= 1) {
               segment.setHorizontalAlignment(SwingConstants.RIGHT);
            }
   ...
   /** Gets bit representation of the value. This will be called from the
    * debugger thread.
    *
    ...
    *
    * @throws ViewerException if an exception occurs while accessing the
    * value. **/
   public abstract long getBits(Value value, DebugContext context)
         throws ViewerException;


   /** Gets the double representation of a value. This will be called from the
    * debugger thread.
    *
    * @param value the value.
    *
   ...
   public Dimension getMainSize() {
      ...
      int w = textSegmentLabels[2].getX() + textSegmentLabels[2].getWidth()
            + textSegmentLabels[0].getX() + mainPanel.getInsets().left
    ...
    *
    * @param mantissa the mantissa or an intermediate value for mantissa
    * computation.
    *
    * @return a string representing the decimal value of <code>mantissa</code>,
    * with an appropriate number of digits for the type that this viewer
    * displays. **/
   ...
   /** Gets text for the value. This will be called from the debugger thread.
    *
    * @param value the value.
    *
    * @param context the current debugger context.
   ...
   public void updateGui() {
      ...
      computationLabel[0]
            .setText("sign = " + ((bits & signMask) == 0L ? '+' : '-'));

      textSegmentLabels[3].setText((bits & signMask) == 0L ? " 0" : " 1");
      textSegmentLabels[4]
            .setText(" " + bitString(bits, firstExponentBit, lastExponentBit));
      textSegmentLabels[5]
            .setText(" " + bitString(bits, firstMantissaBit, lastMantissaBit));

      long exponentMask = (2L << firstExponentBit) - (1L << lastExponentBit);
      long exponent = (bits & exponentMask) >> lastExponentBit;

      long mantissaMask = (2L << firstMantissaBit) - (1L << lastMantissaBit);
      ...
      if (exponent == (2L << firstExponentBit - lastExponentBit) - 1) {
         if (mantissa == 0L) {
            computationLabel[1]
                  .setText("value = " + ((bits & signMask) == 0L ? '+' : '-')
      ...
      BigDecimal bDMantissa = BigDecimal.valueOf(mantissa).stripTrailingZeros();
      BigDecimal div = BigDecimal.valueOf(2);
      try {
         div = div.pow(firstMantissaBit - lastMantissaBit + 1)
               .stripTrailingZeros();
         BigDecimal bdDiv = bDMantissa.divide(div, 200, RoundingMode.HALF_UP)
               .stripTrailingZeros();
         if (exponent == 0L) {
         ...
         if (exponent == 0L) {
            computationLabel[5]
                  .setText("      = " + ((bits & signMask) == 0L ? '+' : '-')
                        + "mantissa * 2 ^ " + (-exponentBias + 1));
         }
         else {
            computationLabel[5]
                  .setText("      = " + ((bits & signMask) == 0L ? '+' : '-')
                        + "mantissa * 2 ^ " + (exponent - exponentBias));
         }
    ...
    *
    * @return a string representation of the bits in <code>bits</code> from
    * <code>firstBit</code> to <code>lastBit</code>. **/
