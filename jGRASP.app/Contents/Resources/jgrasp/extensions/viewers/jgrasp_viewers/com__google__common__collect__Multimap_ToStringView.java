...
/** A guava Multimap viewer that displays the entries as a list of key/value
 * collection strings. **/
public class com__google__common__collect__Multimap_ToStringView
   ...
   public int update(final ViewerValueData valueData,
      ...
      Method entrySetMethod =
            map.getMethod(context, "entrySet", "java.util.Set", null);
      Value entrySet = map.invokeMethod(context, entrySetMethod, null);

      Method toArrayMethod =
            entrySet.getMethod(context, "toArray", "java.lang.Object[]", null);
      ...
      for (int i = 0; i < numItemsShown; i++) {
         if (i + viewOffset < size) {
            ...
            String keyStr = Util.encodeString(keyVal.toString(context), true);
            if (keyStr.length() > 25) {
            ...
            textOut[i] =
                  "_" + (i + viewOffset) + "_ = " + keyStr + ": " + valueStr;
         }
         else {
            textOut[i] = " ";
         }
