...
public abstract class ImageView extends ViewerRoot {
   ...
   private static class Data {
       ...
       *
       * @param pixelDataIn the pixel data, or null if the image is not
       * available.
       *
       ...
       *
       * @param completeIn true if the image data is complete, false
       * otherwise. **/
    ...
    *
    * @param clientTextOut the first element of this array is used to return
    * display text, if any.
    *
   ...
   public void updateGui() {
      ...
      if (data.pixelData == null) {
         ...
         displayImage =
               new BufferedImage(data.w, data.h, BufferedImage.TYPE_INT_ARGB);
   ...
   public void updateState(final ViewerValueData valueData,
      ...
      if (value.isNull()) {
         currentData =
               new Data("Image is null.", clientTextOut[0], null, 0, 0, false);
         return;
      }
      Method getWidthMethod =
            value.getMethod(context, "getWidth", "int", null);
      int width =
            value.invokeMethod(context, getWidthMethod, null).toInt(context);
      Method getHeightMethod =
            value.getMethod(context, "getHeight", "int", null);
      int height =
            value.invokeMethod(context, getHeightMethod, null).toInt(context);

      String dsText = width + " x " + height;

      if (width <= 0 || height <= 0) {
         currentData = new Data(dsText, clientTextOut[0], null, 0, 0, false);
         return;
      }
      Value scaledWidthV =
            context.createPrimitiveValue("int", String.valueOf(width));
      Value scaledHeightV =
            context.createPrimitiveValue("int", String.valueOf(height));
      ...
      if (bufferedImage == null) {
         ...
         Type bufferedImageType =
               context.getType("java.awt.image.BufferedImage");
         bufferedImageType.validate(context);
         Constructor biCons =
               bufferedImageType.getConstructor(context, new String[]
               { "int", "int", "int" });
         bufferedImage =
               bufferedImageType.createInstance(context, biCons, new Value[]
               { scaledWidthV, scaledHeightV, context
                     .createPrimitiveValue("int", "2" /* TYPE_INT_ARGB */) });
         Method getGraphicsMethod = bufferedImageType.getMethod(context,
               "getGraphics", "java.awt.Graphics", null);
         Value graphics =
               bufferedImage.invokeMethod(context, getGraphicsMethod, null);
         Method drawImage =
               graphics.getMethod(context, "drawImage", "boolean", new String[]
               { "java.awt.Image", "int", "int", "int", "int",
                     "java.awt.image.ImageObserver" });
         Value result =
               graphics.invokeMethod(context, drawImage, new Value[]
               { value, zeroV, zeroV, scaledWidthV, scaledHeightV, nullV });
         ...
         List<Field> gFields =
               graphics.getType(context).getFields(Type.INSTANCE);
         for (Field f : gFields) {
      ...
      for (int i = 0; i < pixels.size(); i++) {
         ...
         switch (type) {
            ...
            case BufferedImage.TYPE_INT_BGR:
               pixel = 0xff000000 | (pixel & 0xff0000) >> 16 | pixel & 0xff00
                     | (pixel & 0xff) << 16;
               break;
      ...
      if (createdImage) {
         Method flush =
               bufferedImage.getMethod(context, "flush", "void", null);
      ...
      currentData = new Data(dsText, clientTextOut[0], pixelData, width,
            height, complete);
   }
      ...
      imagePanel = new JPanel() {
         ...
         public Dimension getPreferredSize() {
            ...
            return new Dimension(
                  Math.min(imageWidth + insets.left + insets.right, MAX_WIDTH),
                  Math.min(imageHeight + insets.top + insets.bottom,
                        MAX_HEIGHT));
         }
         ...
         public void paintComponent(final Graphics g) {
            ...
            if (imageWidth == width && imageHeight <= height
               ...
               if (imageWidth > 0 && imageHeight > 0) {
                  drawImage = displayImage.getScaledInstance(imageWidth,
