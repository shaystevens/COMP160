...
/** A viewer that displays a java.lang.String as it could appear in source
 * code. **/
public class java__lang__String_SourceView extends TextAreaView {
   ...
   public java__lang__String_SourceView(final ViewerCreateData vcd) {
   }
   ...
   public String getDisplayText(final Value value, final DebugContext context)
      ...
      for (int start = 0; start < result.length();) {
         ...
         if (end >= result.length()) {
            ...
            if (lastBackslash >= end - 1 || lastBackslash >= end - 6
