...
int main(int argc, char *argv[]) {
   ...
   if (pid == 0) { /* Child process - start jGRASP. */
      ...
      char exePath[MAXPATHLEN + 50];
      if (javaCommand == NULL) {
      #ifdef __linux__
         if (readlink("/proc/self/exe", exePath, MAXPATHLEN) >= 0) {
               ...
               *tmp = '\0';
               ...
               if (tmp != NULL) {
                  *tmp = '\0';
                  tmp = strrchr(exePath, '/');
                  if (tmp != NULL) {
                     sprintf(tmp, "/bundled/java/bin/java", javaCommand);
                     if (stat(exePath, &statBuffer) == 0) {
                        javaCommand = exePath;
                     }
                  }
               }
      ...
      if (jgraspClasspath == NULL) {
         if (javaCommand[0] == '/') {
            ...
            tmp = strrchr(jgraspClasspath, '/');  // Cannot fail.
